    ...
    <section id="features" className="py-12 md:py-24 bg-background">
      <div className="container mx-auto px-4">
        <div className="text-center max-w-3xl mx-auto mb-12">
          <h2 className="text-3xl font-bold tracking-tight md:text-4xl">{t('landingPage.features.title')}</h2>
          <p className="mt-4 text-lg text-muted-foreground">{t('landingPage.features.subtitle')}</p>
        </div>
          ...
          {features.map((featureKey) => {
            ...
            return (
              <div key={featureKey} className="bg-card p-6 rounded-lg border shadow-sm hover:shadow-md transition-shadow">
                ...
                <h3 className="text-xl font-bold mb-2">{t(`landingPage.features.${featureKey}.title`)}</h3>
                <p className="text-muted-foreground mb-4 text-sm">{t(`landingPage.features.${featureKey}.description`)}</p>
                <ul className="space-y-2 text-sm">
                  {(t(`landingPage.features.${featureKey}.points`, { returnObjects: true }) as string[]).map((point, index) => (
