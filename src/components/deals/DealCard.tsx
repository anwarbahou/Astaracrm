  ...
  return (
    <>
      ...
      <NoteModal
        ...
        note={undefined}
        // Pre-attach the deal as the related entity
        // The NoteModal will pick up the deal as relatedEntityType and relatedEntityId
        // via its initial state logic
        // We'll use a key to force re-mount if the deal changes
        key={`note-modal-${deal.id}`}
        // @ts-ignore: We'll need to extend NoteModal to accept initial related entity props if not already
        initialRelatedEntityType="deal"
        initialRelatedEntityId={deal.id}
        initialRelatedEntityName={deal.name}
      />
    </>
  );
}
